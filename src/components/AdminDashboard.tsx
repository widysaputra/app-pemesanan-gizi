...
export const AdminDashboard: React.FC<AdminDashboardProps> = ({
  ...
  const handleUpdateAdminPassword = async (e: React.FormEvent) => {
    ...
    if (newPasswordVal.trim().toLowerCase() === 'admin123') {
      setPwdNotice({ type: 'error', text: 'Kata sandi admin123 telah dinonaktifkan! Silakan gunakan kata sandi lain.' });
      return;
    }
    ...
    try {
      const res = await realtimeService.updateAdminPassword(newPasswordVal.trim());
      ...
      setPwdNotice({ type: 'success', text: res.message || 'Kunci sandi admin berhasil diperbarui dan berlaku di seluruh perangkat!' });
      setTimeout(() => setPwdNotice(null), 5000);
    } catch (err: any) {
      setPwdNotice({ type: 'error', text: err.message || 'Gagal memperbarui sandi admin' });
    } finally {
  ...
  React.useEffect(() => {
    const fetchConfigs = async () => {
      ...
      try {
        ...
        if (pwdData && pwdData.currentPassword) {
          setCurrentAdminPassword(pwdData.currentPassword);
        }
      } catch (err) {
        console.warn('Could not fetch admin password', err);
      }
    };
    fetchConfigs();
  }, []);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-2xl border border-slate-200 p-4 sm:p-5 shadow-xs flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="ml-1.5 inline-flex items-center gap-1.5 px-2.5 py-1 text-[11px] font-bold text-amber-800 bg-amber-50 hover:bg-amber-100 border border-amber-300 rounded-lg transition-colors cursor-pointer"
              title="Klik untuk membuka menu ganti kunci sandi admin"
            >
              <KeyRound className="w-3.5 h-3.5 text-amber-600" />
              <span>Ganti Kunci Admin</span>
            </button>
          </div>
        ...
        {/* Tab Buttons */}
        <div className="flex items-center bg-slate-100 p-1.5 rounded-xl border border-slate-200 shrink-0 flex-wrap gap-1">
          <button
      ...
      {/* ========================================================
          TAB 5: KEAMANAN & KUNCI ADMIN
          ======================================================== */}
      {activeTab === 'security' && (
        <div className="space-y-6">
          
          {/* Header Card */}
          <div className="bg-gradient-to-r from-amber-700 to-amber-900 rounded-2xl p-6 text-white shadow-xs space-y-3">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-xl bg-white/10 flex items-center justify-center border border-white/20">
                <KeyRound className="w-5 h-5 text-amber-300" />
              </div>
              <div>
                <h3 className="text-lg font-bold text-white">Keamanan &amp; Kunci Sandi Administrator</h3>
                <p className="text-xs text-amber-100/90">
                  Ubah kata sandi untuk melindungi dashboard instalasi gizi. Kata sandi ini disimpan di server dan berlaku untuk semua perangkat.
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl border border-slate-200 p-6 shadow-xs space-y-5 max-w-2xl">
            <div className="flex items-center justify-between pb-3 border-b border-slate-100">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <h4 className="font-bold text-slate-900 text-sm">Ganti Kata Sandi Admin</h4>
                  <p className="text-xs text-slate-500">Masukkan kata sandi baru untuk menggantikan sandi lama.</p>
                </div>
              </div>
              <div className="text-xs font-semibold px-2.5 py-1 rounded-full bg-emerald-50 border border-emerald-200 text-emerald-700 flex items-center gap-1.5">
            ...
            {pwdNotice && (
              <div className={`p-3.5 rounded-xl text-xs flex items-center gap-2.5 ${
                ...
                <span className="font-medium">{pwdNotice.text}</span>
              </div>
            )}
            ...
            <form onSubmit={handleUpdateAdminPassword} className="space-y-4">
                ...
                <div>
                  ...
                  <div className="relative">
                    <input
                      ...
                      className="w-full px-3.5 py-2.5 text-xs rounded-xl border border-slate-300 focus:outline-none focus:ring-2 focus:ring-amber-500 pr-10"
                    />
                    <button
                      type="button"
              ...
              <div className="p-3 bg-slate-50 border border-slate-200 rounded-xl space-y-1 text-xs text-slate-600">
                <div className="flex items-center justify-between">
                  <span className="text-slate-500">Kata sandi saat ini:</span>
                  <div className="flex items-center gap-2">
                    <code className="bg-white px-2 py-0.5 rounded border border-slate-200 font-mono font-bold text-slate-800">
                      {currentAdminPassword ? (showNewPassword ? currentAdminPassword : '••••••••') : '(Belum diatur)'}
                    </code>
              ...
              <div className="pt-2 flex items-center justify-end gap-3">
                ...
                <button
                  ...
                  className="px-5 py-2.5 rounded-xl bg-amber-600 hover:bg-amber-700 active:scale-95 text-white text-xs font-bold shadow-sm transition-all cursor-pointer flex items-center gap-2 disabled:opacity-50"
                >
                  {isUpdatingPassword ? (
                    <>
                    ...
                    <>
                      <Check className="w-3.5 h-3.5" />
                      <span>Simpan Kunci Sandi Baru</span>
                    </>
                  )}
                </button>
              </div>
            ...
            <div className="border-t border-slate-100 pt-4 text-xs text-slate-500 space-y-1.5">
              <div className="font-bold text-slate-700">Catatan Keamanan:</div>
              <ul className="list-disc list-inside space-y-1 text-[11px] text-slate-500">
                <li>Kata sandi ini digunakan untuk masuk ke Dashboard Admin pada menu header kanan atas.</li>
                <li>Setelah disimpan, perubahan langsung aktif seketika di semua laptop, PC, dan smartphone perawat/staf instalasi gizi.</li>
                <li>Pastikan mencatat atau mengingat kata sandi ini dengan baik demi keamanan akses sistem instalasi gizi.</li>
              </ul>
            </div>
          </div>
