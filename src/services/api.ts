...
export function getLocalFonnteConfig(): {
  ...
  const DEFAULT_TARGET = '081573570843';
...
export class HospitalRealtimeService {
  ...
  async createOrder(payload: {
      ...
      if (res.ok) {
        ...
        if (result && result.order && result.order.id) {
          ...
          if (!result.waSent && fonnteConfig.token) {
            try {
              const target = fonnteConfig.targetNumber || '081573570843';
    ...
    if (fonnteConfig.token) {
      try {
        const targetList = [fonnteConfig.targetNumber || '081573570843'];
        const cleanPat = (payload.phoneNumber || '').replace(/[^0-9]/g, '');
        const cleanAdm = (fonnteConfig.targetNumber || '081573570843').replace(/[^0-9]/g, '');
        ...
        if (directData.status === true || directData.status === 'true') {
          waSent = true;
          waStatusText = `Terkirim langsung ke WhatsApp Admin Gizi (${fonnteConfig.targetNumber || '081573570843'}) via Fonnte Gateway`;
        } else {
    ...
    const newOrder: HospitalOrder = {
      ...
      whatsappNotification: {
        sent: waSent,
        targetNumber: fonnteConfig.targetNumber || '081573570843',
